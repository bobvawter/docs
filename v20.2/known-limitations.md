...
- CockroachDB does not support the `@` operator for [spatial features](spatial-features). Instead of using `@` in spatial expressions, we recommend using the inverse, with `~`. For example, instead of `a @ b`, use `b ~ a`.
